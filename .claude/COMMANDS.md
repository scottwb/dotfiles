...
- **Model pins are TIER ALIASES, not vendors.** The `model:` values at spawn sites (`"opus"`, `"fable"`) are Claude Code tier aliases. Each resolves through the corresponding `ANTHROPIC_DEFAULT_<TIER>_MODEL` environment variable, so the vendor mapping lives one layer down and never in the command files. **Do not add per-provider fallback logic to the command files.** The indirection already exists; duplicating it there would be a regression, not a feature. What the pins do assert is a capability floor: an audit spawned at the Fable tier must actually be the strongest model available, whoever makes it.
- **Where the vendor mapping lives:** in `bin/claude-run`, not in `settings.json`. The parked `x-env` Ollama profile was retired on 2026-08-01 in favour of per-process launchers (`claude-glm`, `claude-gpt`, `claude-ollama`, `claude-openrouter`). Routing is per process now, so a routed session and a plain Max session coexist and `settings.json` never needs editing to switch backends.
- **Routed profiles leave the audit tier UNMAPPED, on purpose.** This is the answer to the `DECIDE-ME` marker that sat in `settings.json`, and it is the one interaction between the launchers and this kit that matters. `ANTHROPIC_DEFAULT_FABLE_MODEL` is deliberately not set by any routed profile, so a `/phasegate`, a security pass, or any Fable-tier spawn **halts** under routing rather than silently running the audit on a local 30b or a cheap hosted model. That is the STOP-not-degrade rule working as intended, not a misconfiguration to fix.

  Verified rather than assumed (Gate A Check 4a, 2026-07-30): with the variable unset, `--model fable` resolves to the literal `claude-fable-5`, which a routed backend rejects with a hard error and exit 1. It does **not** fall through to the Opus or Sonnet mapping, which was the dangerous case.

  Practical consequence: **run gates and security passes on a plain Anthropic session.** If you want one under a routed profile, map the audit tier to something genuinely strong and say so explicitly; do not let it fall through unnoticed.
