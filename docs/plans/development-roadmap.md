...
- ~~**More OpenRouter models** (`kimi`, `deepseek`, `qwen`)~~ **Retired 2026-09-10, superseded by `--provider`/`--model`.** Arbitrary models now work with nothing pre-configured, so there is no alias to add: `claude-run --model deepseek/deepseek-v4.1-flash` resolves its own window from the catalog. **Still open from this item:** the `glm` alias collision, since GLM exists on both backends and the alias resolves to Ollama. **The ZDR warning here was wrong and is retracted (measured 2026-09-10):** authenticated and public `/api/v1/models` both return 436 models with nothing hidden, so Zero Data Retention does not filter the catalog. It is enforced when a request is routed, not when the catalog is listed. Lower value than it looks: Gate B Check 5 showed gateway discovery already reaches the whole filtered catalog in-session, making these a convenience rather than the way to access a model.
