...
| **A11** | **The transcript store is unwritable by construction, and `--force` does not override it.** `check_destination` refuses any target whose realpath falls inside `~/.claude/projects/`, before any directory is created. Added after the Phase 3 gate demonstrated that `-o <transcript>.jsonl --force` replaced a transcript with HTML, and that the no-overwrite error coached the user into doing exactly that. The plan had stated the property since day one; nothing had enforced it. |
| **A12** | **Sender attribution turns on the entrypoint, and says `caller` when it does not know.** An `sdk-cli` session was not a human typing, so naming the configured human would be a false attribution in an audit log. Added after the Phase 3 gate found daily briefs rendering as `scott-to-greenthumb`. |
